...
const my_file = './results_no_cache_leaflet.csv';
...
async function measurements(driver, url) {
    ...
    try {
        ...
        const RAM_on_end = os.freemem() / 1024 / 1024; 
...
async function get_gpu_stats() {
    return new Promise((resolve, reject) => {
        exec('nvidia-smi --query-gpu=temperature.gpu,utilization.gpu --format=csv,noheader', (error, stdout, stderr) => {
            ...
            const [temperature, usage_percentage] = stdout.trim().split(',');

            resolve({ temperature, utilization: usage_percentage });
        });
    });
}
...
async function wait_for_tilesloaded(driver) {
    await driver.wait(until.elementLocated(By.css('body[data-leaflet-loaded="true"]')), 10000);
}
...
async function count_network_req(driver) {
    ...
    const googleMapsEntries = entries.filter(entry => entry.name.includes('leaflet'));
...
async function measure_latency(driver) {
    const latency_logs = await driver.executeScript(`
        const entries = window.performance.getEntriesByType('resource');
        return entries.filter(entry => entry.name.includes('leaflet')).map(entry => ({
            name: entry.name,
...
async function action_after_tests() {
    try {
        ...
        console.log(`Opened Chrome browser and navigated to ${url_2}`);
    } catch (error) {
        console.error('An error occurred while opening Chrome and navigating to the URL:', error);
    }
}



async function run_tests(requestsPerBrowser) {
    ...
    const urls = [
        'file:///C:/Users/Anwender/thesis_webapp/index_leaflet_normal.html',
        'file:///C:/Users/Anwender/thesis_webapp/leaflet_stresstest.html'
    ];
